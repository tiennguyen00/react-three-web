...
const CameraControlTheatre = () => {
  ...
  const handleWheeling = (e: any) => {
    ...
    if (direction < 0) nextStop = stops.find((stop) => stop > currentSheet.sequence.position)
    else nextStop = stops.findLast((stop) => stop < currentSheet.sequence.position)

    if (
      (currentSheet.sequence.position === stops[0] && direction > 0) ||
      (currentSheet.sequence.position === stops[stops.length - 1] && direction < 0)
    )
      return
    ...
    if (nextStop === undefined)
      ...
      currentSheet.sequence.play({
        range: direction < 0 ? [currentSheet.sequence.position, nextStop] : [nextStop, currentSheet.sequence.position],
  ...
  useEffect(() => {
    ...
    const unsubscribe = theatreObject.onValuesChange((newValues) => {
      if (threeRef.current) {
        threeRef.current.lookAt(new THREE.Vector3(newValues.lookAt.x, newValues.lookAt.y, newValues.lookAt.z))
      }
    })
    return unsubscribe
  }, [theatreObject])
